...
def alumni_required(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url='login'):
    ...
    '''
    actual_decorator = user_passes_test(lambda u: u.is_active and u.is_alumni,
                                        login_url=login_url,
...
def faculty_required(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url='login'):
    ...
    redirects to the log-in page if necessary.
    '''
    actual_decorator = user_passes_test(lambda u: u.is_active and u.is_faculty,
                                        login_url=login_url,
                                        redirect_field_name=redirect_field_name)
    if function:
        return actual_decorator(function)
...
def verify_required(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url='/verify'):
    decorator = user_passes_test(lambda u: (u.is_verified and u.is_active) or u.is_superuser,
                                 login_url=login_url,
                                 redirect_field_name=redirect_field_name)
    if function:
